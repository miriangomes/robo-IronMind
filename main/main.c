...
#define DELAY_INICIAL_OBRIGATORIO_MS 5000 
...
void mov_tras() {
    ...
    gpio_set_level(MOTOR_A_IN1, 0); gpio_set_level(MOTOR_A_IN2, 1);
    gpio_set_level(MOTOR_B_IN1, 0); gpio_set_level(MOTOR_B_IN2, 1);
    set_speed(MAX_SPEED, MAX_SPEED);
}

void mov_frente() {
    if (!combate_autorizado) return;
    gpio_set_level(MOTOR_A_IN1, 1); gpio_set_level(MOTOR_A_IN2, 0);
    gpio_set_level(MOTOR_B_IN1, 1); gpio_set_level(MOTOR_B_IN2, 0);
    set_speed(MAX_SPEED, MAX_SPEED);
}
...
void app_main(void) {
    ...
    while (1) {
        ...
        // --- PRIORIDADE 1: NÃO CAIR DO DOHYO ---
